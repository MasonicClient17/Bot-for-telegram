import asyncio, logging, math, os, re, random
...
# Инициализируем глобальный пул соединений
DB_POOL = None
...
async def db_query(sql, *params, fetchone=False, fetchall=False):
...
async def init_db():
    global DB_POOL
    # Создаем пул соединений (максимум 10 параллельных соединений)
    DB_POOL = await asyncpg.create_pool(DB_URL, min_size=1, max_size=10)
    
    # Регулярные Postgres-запросы в asyncpg используют $1, $2 вместо %s
    async with DB_POOL.acquire() as conn:
        await conn.execute("""
...
async def get_user_lvl(m: Message, uid: int) -> int:
    ...
    res = await db_query("SELECT role_level FROM mod_roles WHERE chat_id=$1 AND user_id=$2", m.chat.id, uid, fetchone=True)
    db_lvl = res['role_level'] if res else 0
    return db_lvl if cm.status == "administrator" else (db_lvl if db_lvl > 0 else 0)

async def get_name(chat_id: int, user_id: int, default: str) -> str:
    res = await db_query("SELECT nickname FROM custom_nicknames WHERE chat_id=$1 AND user_id=$2", chat_id, user_id, fetchone=True)
    return res['nickname'] if res else default
...
async def resolve_target(m: Message):
    ...
    for w in words:
        if w.startswith("@"):
            ...
            res = await db_query("SELECT user_id, first_name FROM user_activity WHERE chat_id=$1 AND LOWER(username)=$2", m.chat.id, uname, fetchone=True)
            if res: return res['user_id'], res['first_name'], w
            return None, w, w
...
async def set_role_name(m: Message):
    ...
    r_row = await db_query("SELECT r1, r2, r3 FROM role_names WHERE chat_id=$1", m.chat.id, fetchone=True)
    r = [r_row['r1'], r_row['r2'], r_row['r3']] if r_row else ["Хелпер", "Модер", "Админ"]
    r[int(p[1]) - 1] = p[2].strip()
    await db_query("INSERT INTO role_names VALUES ($1,$2,$3,$4) ON CONFLICT(chat_id) DO UPDATE SET r1=EXCLUDED.r1, r2=EXCLUDED.r2, r3=EXCLUDED.r3", m.chat.id, r[0], r[1], r[2])
...
async def promote(m: Message):
    ...
    r_row = await db_query("SELECT r1, r2, r3 FROM role_names WHERE chat_id=$1", m.chat.id, fetchone=True)
    r = [r_row['r1'], r_row['r2'], r_row['r3']] if r_row else ["Хелпер", "Модер", "Админ"]
    await db_query("INSERT INTO mod_roles VALUES ($1,$2,$3) ON CONFLICT(chat_id, user_id) DO UPDATE SET role_level=EXCLUDED.role_level", m.chat.id, tid, target_lvl)
    if target_lvl >= 2:
...
async def demote(m: Message):
    ...
    await db_query("DELETE FROM mod_roles WHERE chat_id=$1 AND user_id=$2", m.chat.id, tid)
...
async def set_rules(m: Message):
    ...
    await db_query("INSERT INTO chat_rules VALUES ($1,$2) ON CONFLICT(chat_id) DO UPDATE SET rules_text=EXCLUDED.rules_text", m.chat.id, p[1])
...
async def get_rules(m: Message):
    res = await db_query("SELECT rules_text FROM chat_rules WHERE chat_id=$1", m.chat.id, fetchone=True)
    await m.answer(f"📜 **Устав сада:**\n\n{res['rules_text']}" if res and res['rules_text'] else "📜 Устав сада еще не установлен.")
...
async def set_nick(m: Message):
    ...
    await db_query("INSERT INTO custom_nicknames VALUES ($1,$2,$3) ON CONFLICT(chat_id, user_id) DO UPDATE SET nickname=EXCLUDED.nickname", m.chat.id, tid, p[1].strip())
...
async def rem_nick(m: Message):
    ...
    await db_query("DELETE FROM custom_nicknames WHERE chat_id=$1 AND user_id=$2", m.chat.id, tid)
...
async def list_nicks(m: Message):
    ...
    total_res = await db_query("SELECT COUNT(*) FROM custom_nicknames WHERE chat_id=$1", m.chat.id, fetchone=True)
    total = total_res['count'] if total_res else 0
    if not total: return await m.answer("📝 Ни у кого нет кастомных ников.")
    pages = math.ceil(total / 10)
    page = min(page, pages)
    rows = await db_query("SELECT n.user_id, n.nickname, a.first_name FROM custom_nicknames n LEFT JOIN user_activity a ON n.chat_id=a.chat_id AND n.user_id=a.user_id WHERE n.chat_id=$1 ORDER BY n.nickname LIMIT 10 OFFSET $2", m.chat.id, (page-1)*10, fetchall=True)
    msg = f"🏷 **Ники чата ({page}/{pages}):**\n\n" + "\n".join([f"{i}. **{r['nickname']}** *({r['first_name'] or 'ID:'+str(r['user_id'])})*" for i, r in enumerate(rows, (page-1)*10 + 1)])
...
async def add_cmd(m: Message):
    ...
    if await db_query("SELECT command_name FROM custom_commands WHERE chat_id=$1 AND LOWER(command_name)=$2", m.chat.id, c_name, fetchone=True):
        return await m.answer(f"⚠️ Команда `{c_name}` уже существует! Сначала удалите её через `-команда {c_name}`.")
    await db_query("INSERT INTO custom_commands VALUES ($1,$2,$3)", m.chat.id, c_name, p[2])
...
async def del_cmd(m: Message):
    ...
    c_name = p[1].lower().strip()
    await db_query("DELETE FROM custom_commands WHERE chat_id=$1 AND LOWER(command_name)=$2", m.chat.id, c_name)
...
async def list_cmds(m: Message):
    cmds = await db_query("SELECT command_name FROM custom_commands WHERE chat_id=$1 ORDER BY command_name", m.chat.id, fetchall=True)
    if not cmds: return await m.answer("📝 В чате нет кастомных РП-команд.")
    await m.answer("📜 **РП-команды:**\n\n" + "\n".join([f"• `{c['command_name']}`" for c in cmds]), parse_mode="Markdown")
...
async def process_msg(m: Message):
    ...
    await db_query("INSERT INTO user_activity VALUES ($1,$2,$3,$4,1) ON CONFLICT(chat_id, user_id) DO UPDATE SET message_count=user_activity.message_count+1, username=EXCLUDED.username, first_name=EXCLUDED.first_name", m.chat.id, m.from_user.id, m.from_user.username, m.from_user.first_name)
    ...
    if lvl >= 2:
        if t.startswith("варн") or t.startswith("отругать"):
            ...
            if tid:
                ...
                res = await db_query("SELECT warn_count FROM warn_system WHERE chat_id=$1 AND user_id=$2", m.chat.id, tid, fetchone=True)
                warns = (res['warn_count'] if res else 0) + 1
                if warns >= 3:
                    await db_query("DELETE FROM warn_system WHERE chat_id=$1 AND user_id=$2", m.chat.id, tid)
                    try:
                        await m.chat.ban(tid)
                        return await m.answer(f"🫙 **{tname}** получил [3/3] варнов и забанен!{r_str}", parse_mode="Markdown")
                    except Exception as e: return await m.answer(f"Ошибка: {e}")
                else:
                    await db_query("INSERT INTO warn_system VALUES ($1,$2,$3) ON CONFLICT(chat_id, user_id) DO UPDATE SET warn_count=EXCLUDED.warn_count", m.chat.id, tid, warns)
                    return await m.answer(f"⚠️ **{tname}** получил предупреждение! [{warns}/3]{r_str}", parse_mode="Markdown")
        elif t in ["актив", "стата", "статистика"]:
            top = await db_query("SELECT first_name, message_count FROM user_activity WHERE chat_id=$1 ORDER BY message_count DESC LIMIT 20", m.chat.id, fetchall=True)
            if not top: return await m.answer("Статистика пуста.")
            return await m.answer("📊 **ТОП-20 участников:**\n\n" + "\n".join([f"{i}. {r['first_name']} — {r['message_count']} сообщ." for i, r in enumerate(top, 1)]), parse_mode="Markdown")
    ...
    # РП-КОМАНДЫ
    cmd = await db_query("SELECT response_text FROM custom_commands WHERE chat_id=$1 AND LOWER(command_name)=$2", m.chat.id, t, fetchone=True)
    if cmd:
        sender = await get_name(m.chat.id, m.from_user.id, m.from_user.first_name if m.from_user else "Кто-то")
        tid, tname, _ = await resolve_target(m)
        reply_user = await get_name(m.chat.id, tid, tname) if tid else "кого-то"
        reply_txt = (m.reply_to_message.text or m.reply_to_message.caption or "") if m.reply_to_message else ""
        
        users = await db_query("SELECT user_id, first_name FROM user_activity WHERE chat_id=$1", m.chat.id, fetchall=True)
        chosen = random.choice(users) if users else None
        rand_user = await get_name(m.chat.id, chosen['user_id'], chosen['first_name']) if chosen else "Кто-то"
...
async def main():
    await init_db()
    logging.info("🚀 Бот запущен с асинхронным asyncpg пулом Supabase!")
