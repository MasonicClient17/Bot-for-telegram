...
def parse_time(time_str: str):
    ...
    match = re.match(
        r"^(\d+)\s*(мин|мин.|минут|минуты|ч|час|часа|часов|д|ден|день|дня|дней)?$",
...
async def set_custom_nickname(message: Message):
    ...
    if len(parts) < 2:
        await message.answer(
            "Использование: `+ник [твой ник]`", parse_mode="Markdown"
        )
        return
    ...
    if message.reply_to_message:
        if not await is_admin(message):
            await message.answer(
                "⚠️ Менять ники другим участникам могут только администраторы."
            )
            return
...
async def remove_custom_nickname(message: Message):
    ...
    if message.reply_to_message:
        if not await is_admin(message):
            await message.answer(
                "⚠️ Сбрасывать ники другим участникам могут только администраторы."
            )
            return
        target_user = message.reply_to_message.from_user
    else:
    ...
    if deleted > 0:
        await message.answer(
            f"🗑 РП-ник пользователя {target_user.first_name} сброшен."
        )
    else:
        await message.answer(
            f"⚠️ У {target_user.first_name} не было установленного РП-ника."
        )


# === СПИСОК НИКНЕЙМОВ / ЗВАНИЙ (с пагинацией) ===
@dp.message(
    F.text.lower().startswith("ники") | F.text.lower().startswith("звания")
)
async def list_custom_nicknames(message: Message):
    ...
    cursor.execute(
        "SELECT COUNT(*) FROM custom_nicknames WHERE chat_id = ?", (chat_id,)
    )
    ...
    if total_count == 0:
        conn.close()
        await message.answer(
            "📝 В этом чате пока ни у кого нет кастомных РП-ников."
        )
        return
    ...
    for idx, (uid, nick, original_name) in enumerate(
        nicknames, start=offset + 1
    ):
...
async def add_custom_command(message: Message):
    ...
    await message.answer(
        f"✅ Команда `{cmd_name}` успешно сохранена!", parse_mode="Markdown"
    )
...
async def delete_custom_command(message: Message):
    ...
    if len(parts) < 2:
        await message.answer(
            "Использование: `-команда [название]`", parse_mode="Markdown"
        )
        return
    ...
    if deleted > 0:
        await message.answer(
            f"🗑 Команда `{cmd_name}` удалена!", parse_mode="Markdown"
        )
    else:
        await message.answer(
            f"⚠️ Команда `{cmd_name}` не найдена.", parse_mode="Markdown"
        )
...
async def list_custom_commands(message: Message):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT command_name FROM custom_commands WHERE chat_id = ? ORDER BY"
        " command_name ASC",
        (message.chat.id,),
    )
    ...
    if not commands:
        await message.answer(
            "📝 В этом чате пока нет созданных кастомных команд."
        )
        return
...
async def process_all_messages(message: Message):
    ...
    # --- ИГНОРИРУЕМ СЛУЖЕБНЫЕ КОМАНДЫ ДЛЯ ИСКЛЮЧЕНИЯ ДВОЙНЫХ ОТВЕТОВ ---
    if text.startswith(
        (
            "+ник",
            "-ник",
    ...
    if text in [
        ...
        "ники",
        "звания",
    ] or text.startswith(("ники ", "звания ")):
        return
    ...
    if await is_admin(message):
        ...
        if text in ["банка с джемом", "в банку"] and message.reply_to_message:
            ...
            try:
                ...
                await message.answer(
                    f"🫙 Пользователь {target_user.first_name} отправлен в"
                    " банку с джемом (забанен)!"
                )
        ...
        # РАЗБАН ПО USERNAME: "вытащить из банки @username"
            ...
            if len(parts) > 3 and parts[3].startswith("@"):
                ...
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT user_id FROM user_activity WHERE chat_id = ? AND"
                    " LOWER(username) = ?",
                ...
                if res:
                    try:
                        ...
                        await message.answer(
                            f"🔓 Пользователь @{target_username} вытащен из"
                            " банки (разбанен)!"
                        )
                    except Exception as e:
        ...
        # МУТ НА 1 ДЕНЬ: "дать плод всей боли"
            ...
            try:
                ...
                await message.answer(
                    f"🍎 {target_user.first_name} вкусил плод всей боли и"
                    " замолк на 1 день."
                )
            except Exception as e:
        ...
        # НАСТРАИВАЕМЫЙ МУТ: "клиновый сироп [время]" / "напоить сиропом [время]"
            ...
            text.startswith("клиновый сироп")
            or text.startswith("напоить сиропом")
        ) and message.reply_to_message:
            target_user = message.reply_to_message.from_user
            time_part = (
                text.replace("клиновый сироп", "")
                .replace("напоить сиропом", "")
                .strip()
            )
            ...
            try:
                ...
                await message.answer(
                    f"🍁 {target_user.first_name} напоен кленовым сиропом и"
                    f" молчит {display_time}."
                )
            except Exception as e:
                await message.answer(f"Не удалось выдать мут: {e}")
            return
        ...
        # СНЯТИЕ МУТА: "дать воды"
            ...
            try:
                ...
                await message.answer(
                    f"💧 {target_user.first_name} получил воды и снова может"
                    " говорить!"
                )
            except Exception as e:
        ...
        # ВАРН: "варн" или "отругать"
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT warn_count FROM warn_system WHERE chat_id = ? AND"
                " user_id = ?",
                (chat_id, target_user.id),
            )
            res = cursor.fetchone()
            ...
            if current_warns >= 3:
                ...
                try:
                    await message.chat.ban(target_user.id)
                    await message.answer(
                        f"🫙 {target_user.first_name} получил [3/3] варнов и"
                        " отправлен в банку с джемом!"
                    )
                except Exception as e:
        ...
        # СТАТИСТИКА: "актив", "стата", "статистика"
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT first_name, message_count FROM user_activity WHERE"
                " chat_id = ? ORDER BY message_count DESC LIMIT 20",
                (chat_id,),
            )
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT response_text FROM custom_commands WHERE chat_id = ? AND"
        " command_name = ?",
        (chat_id, text),
    )
    ...
    if custom_cmd:
        ...
        user_first_name = (
            message.from_user.first_name if message.from_user else "Кто-то"
        )
        ...
        if message.reply_to_message:
            if message.reply_to_message.from_user:
                ...
                reply_user_name = get_display_name(
                    chat_id, r_user.id, r_user.first_name
                )
