import asyncio, logging, math, os, re, sqlite3
...
def init_db():
    with sqlite3.connect(DB) as c:
        c.executescript("""
            CREATE TABLE IF NOT EXISTS user_activity (chat_id INT, user_id INT, username TEXT, first_name TEXT, message_count INT DEFAULT 0, PRIMARY KEY (chat_id, user_id));
            CREATE TABLE IF NOT EXISTS warn_system (chat_id INT, user_id INT, warn_count INT DEFAULT 0, PRIMARY KEY (chat_id, user_id));
            ...
            CREATE TABLE IF NOT EXISTS custom_nicknames (chat_id INT, user_id INT, nickname TEXT, PRIMARY KEY (chat_id, user_id));
        """)
...
def get_name(chat_id: int, user_id: int, default: str) -> str:
    with sqlite3.connect(DB) as c:
        res = c.execute("SELECT nickname FROM custom_nicknames WHERE chat_id=? AND user_id=?", (chat_id, user_id)).fetchone()
...
async def set_nick(m: Message):
    ...
    with sqlite3.connect(DB) as c:
        c.execute("INSERT INTO custom_nicknames VALUES (?,?,?) ON CONFLICT(chat_id, user_id) DO UPDATE SET nickname=excluded.nickname", (m.chat.id, target.id, p[1].strip()))
...
async def rem_nick(m: Message):
    ...
    with sqlite3.connect(DB) as c:
        cur = c.execute("DELETE FROM custom_nicknames WHERE chat_id=? AND user_id=?", (m.chat.id, target.id))
...
async def list_nicks(m: Message):
    ...
    page = max(1, page)
    with sqlite3.connect(DB) as c:
        total = c.execute("SELECT COUNT(*) FROM custom_nicknames WHERE chat_id=?", (m.chat.id,)).fetchone()[0]
        if not total: return await m.answer("📝 Ни у кого нет кастомных ников.")
        pages = math.ceil(total / 10)
        page = min(page, pages)
        rows = c.execute("SELECT n.user_id, n.nickname, a.first_name FROM custom_nicknames n LEFT JOIN user_activity a ON n.chat_id=a.chat_id AND n.user_id=a.user_id WHERE n.chat_id=? ORDER BY n.nickname LIMIT 10 OFFSET ?", (m.chat.id, (page-1)*10)).fetchall()
...
async def add_cmd(m: Message):
    ...
    with sqlite3.connect(DB) as c:
        c.execute("INSERT INTO custom_commands VALUES (?,?,?) ON CONFLICT(chat_id, command_name) DO UPDATE SET response_text=excluded.response_text", (m.chat.id, p[1].lower(), p[2]))
...
async def del_cmd(m: Message):
    ...
    with sqlite3.connect(DB) as c:
        del_cnt = c.execute("DELETE FROM custom_commands WHERE chat_id=? AND command_name=?", (m.chat.id, p[1].lower().strip())).rowcount
...
async def list_cmds(m: Message):
    if m.chat.type in ["private"]: return
    with sqlite3.connect(DB) as c:
        cmds = c.execute("SELECT command_name FROM custom_commands WHERE chat_id=? ORDER BY command_name", (m.chat.id,)).fetchall()
...
async def process_msg(m: Message):
    ...
    with sqlite3.connect(DB) as c:
        c.execute("INSERT INTO user_activity VALUES (?,?,?,?,1) ON CONFLICT(chat_id, user_id) DO UPDATE SET message_count=message_count+1, username=excluded.username, first_name=excluded.first_name", (m.chat.id, m.from_user.id, m.from_user.username, m.from_user.first_name))
    ...
    if await is_admin(m):
        if t in ["банка с джемом", "в банку"] and m.reply_to_message:
            ...
            if len(p) > 3 and p[3].startswith("@"):
                with sqlite3.connect(DB) as c:
                    res = c.execute("SELECT user_id FROM user_activity WHERE chat_id=? AND LOWER(username)=?", (m.chat.id, p[3].replace("@",""))).fetchone()
                if res:
                    try:
            ...
            with sqlite3.connect(DB) as c:
                res = c.execute("SELECT warn_count FROM warn_system WHERE chat_id=? AND user_id=?", (m.chat.id, u.id)).fetchone()
                warns = (res[0] if res else 0) + 1
                if warns >= 3:
                    c.execute("DELETE FROM warn_system WHERE chat_id=? AND user_id=?", (m.chat.id, u.id))
                    try:
                        await m.chat.ban(u.id)
                        await m.answer(f"🫙 {u.first_name} получил [3/3] варнов и забанен!")
                    except Exception as e: await m.answer(f"Ошибка: {e}")
                else:
                    c.execute("INSERT INTO warn_system VALUES (?,?,?) ON CONFLICT(chat_id, user_id) DO UPDATE SET warn_count=excluded.warn_count", (m.chat.id, u.id, warns))
                    await m.answer(f"⚠️ {u.first_name} получил предупреждение! [{warns}/3]")
            return
        elif t in ["актив", "стата", "статистика"]:
            with sqlite3.connect(DB) as c:
                top = c.execute("SELECT first_name, message_count FROM user_activity WHERE chat_id=? ORDER BY message_count DESC LIMIT 20", (m.chat.id,)).fetchall()
    ...
    with sqlite3.connect(DB) as c:
        cmd = c.execute("SELECT response_text FROM custom_commands WHERE chat_id=? AND command_name=?", (m.chat.id, t)).fetchone()
    if cmd:
