...
async def get_user_lvl(m: Message, uid: int) -> int:
    ...
    res = db_query("SELECT role_level FROM mod_roles WHERE chat_id=%s AND user_id=%s", (m.chat.id, uid), fetchone=True)
...
def get_name(chat_id: int, user_id: int, default: str) -> str:
    res = db_query("SELECT nickname FROM custom_nicknames WHERE chat_id=%s AND user_id=%s", (chat_id, user_id), fetchone=True)
...
async def resolve_target(m: Message):
    ...
    for w in words:
        if w.startswith("@"):
            ...
            res = db_query("SELECT user_id, first_name FROM user_activity WHERE chat_id=%s AND LOWER(username)=%s", (m.chat.id, uname), fetchone=True)
...
async def set_role_name(m: Message):
    ...
    r = list(db_query("SELECT r1, r2, r3 FROM role_names WHERE chat_id=%s", (m.chat.id,), fetchone=True) or ("Хелпер", "Модер", "Админ"))
    r[int(p[1]) - 1] = p[2].strip()
    db_query("INSERT INTO role_names VALUES (%s,%s,%s,%s) ON CONFLICT(chat_id) DO UPDATE SET r1=EXCLUDED.r1, r2=EXCLUDED.r2, r3=EXCLUDED.r3", (m.chat.id, *r), commit=True)
...
async def promote(m: Message):
    ...
    r = db_query("SELECT r1, r2, r3 FROM role_names WHERE chat_id=%s", (m.chat.id,), fetchone=True) or ("Хелпер", "Модер", "Админ")
    db_query("INSERT INTO mod_roles VALUES (%s,%s,%s) ON CONFLICT(chat_id, user_id) DO UPDATE SET role_level=EXCLUDED.role_level", (m.chat.id, tid, target_lvl), commit=True)
    if target_lvl >= 2:
...
async def demote(m: Message):
    ...
    db_query("DELETE FROM mod_roles WHERE chat_id=%s AND user_id=%s", (m.chat.id, tid), commit=True)
...
async def set_rules(m: Message):
    ...
    db_query("INSERT INTO chat_rules VALUES (%s,%s) ON CONFLICT(chat_id) DO UPDATE SET rules_text=EXCLUDED.rules_text", (m.chat.id, p[1]), commit=True)
...
async def get_rules(m: Message):
    res = db_query("SELECT rules_text FROM chat_rules WHERE chat_id=%s", (m.chat.id,), fetchone=True)
...
async def set_nick(m: Message):
    ...
    db_query("INSERT INTO custom_nicknames VALUES (%s,%s,%s) ON CONFLICT(chat_id, user_id) DO UPDATE SET nickname=EXCLUDED.nickname", (m.chat.id, tid, p[1].strip()), commit=True)
...
async def rem_nick(m: Message):
    ...
    db_query("DELETE FROM custom_nicknames WHERE chat_id=%s AND user_id=%s", (m.chat.id, tid), commit=True)
...
async def list_nicks(m: Message):
    ...
    total = (db_query("SELECT COUNT(*) FROM custom_nicknames WHERE chat_id=%s", (m.chat.id,), fetchone=True) or [0])[0]
    if not total: return await m.answer("📝 Ни у кого нет кастомных ников.")
    pages = math.ceil(total / 10)
    page = min(page, pages)
    rows = db_query("SELECT n.user_id, n.nickname, a.first_name FROM custom_nicknames n LEFT JOIN user_activity a ON n.chat_id=a.chat_id AND n.user_id=a.user_id WHERE n.chat_id=%s ORDER BY n.nickname LIMIT 10 OFFSET %s", (m.chat.id, (page-1)*10), fetchall=True)
...
async def add_cmd(m: Message):
    ...
    if db_query("SELECT command_name FROM custom_commands WHERE chat_id=%s AND LOWER(command_name)=%s", (m.chat.id, c_name), fetchone=True):
        return await m.answer(f"⚠️ Команда `{c_name}` уже существует! Сначала удалите её через `-команда {c_name}`.")
    db_query("INSERT INTO custom_commands VALUES (%s,%s,%s)", (m.chat.id, c_name, p[2]), commit=True)
...
async def del_cmd(m: Message):
    ...
    c_name = p[1].lower().strip()
    db_query("DELETE FROM custom_commands WHERE chat_id=%s AND LOWER(command_name)=%s", (m.chat.id, c_name), commit=True)
...
async def list_cmds(m: Message):
    cmds = db_query("SELECT command_name FROM custom_commands WHERE chat_id=%s ORDER BY command_name", (m.chat.id,), fetchall=True)
...
async def process_msg(m: Message):
    ...
    db_query("INSERT INTO user_activity VALUES (%s,%s,%s,%s,1) ON CONFLICT(chat_id, user_id) DO UPDATE SET message_count=user_activity.message_count+1, username=EXCLUDED.username, first_name=EXCLUDED.first_name", (m.chat.id, m.from_user.id, m.from_user.username, m.from_user.first_name), commit=True)
    ...
    if lvl >= 2:
        if t.startswith("варн") or t.startswith("отругать"):
            ...
            if tid:
                ...
                res = db_query("SELECT warn_count FROM warn_system WHERE chat_id=%s AND user_id=%s", (m.chat.id, tid), fetchone=True)
                warns = (res[0] if res else 0) + 1
                if warns >= 3:
                    db_query("DELETE FROM warn_system WHERE chat_id=%s AND user_id=%s", (m.chat.id, tid), commit=True)
                    try:
                        await m.chat.ban(tid)
                        return await m.answer(f"🫙 **{tname}** получил [3/3] варнов и забанен!{r_str}", parse_mode="Markdown")
                    except Exception as e: return await m.answer(f"Ошибка: {e}")
                else:
                    db_query("INSERT INTO warn_system VALUES (%s,%s,%s) ON CONFLICT(chat_id, user_id) DO UPDATE SET warn_count=EXCLUDED.warn_count", (m.chat.id, tid, warns), commit=True)
                    return await m.answer(f"⚠️ **{tname}** получил предупреждение! [{warns}/3]{r_str}", parse_mode="Markdown")
        elif t in ["актив", "стата", "статистика"]:
            top = db_query("SELECT first_name, message_count FROM user_activity WHERE chat_id=%s ORDER BY message_count DESC LIMIT 20", (m.chat.id,), fetchall=True)
    ...
    # РП-КОМАНДЫ (С ЗАМЕНОЙ ТЕГОВ БЕЗ УЧЕТА РЕГИСТРА)
    cmd = db_query("SELECT response_text FROM custom_commands WHERE chat_id=%s AND LOWER(command_name)=%s", (m.chat.id, t), fetchone=True)
    if cmd:
        ...
        users = db_query("SELECT user_id, first_name FROM user_activity WHERE chat_id=%s", (m.chat.id,), fetchall=True)
